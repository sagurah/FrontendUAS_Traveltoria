    ...
    <v-main>
        ...
        <v-card elevation="3" class="mt-5 mx-6" style="border-radius: 6px;">
            <v-row justify="center" align="center" style="margin:3px auto;">
                ...
                <v-col>
                    <v-btn class="font-weight-bold" style="margin:10px auto;font-family: Poppins; font-size: 20px; text-transform: capitalize; float:right; color: black" x-large color="teal" @click="dialog = true">ADD KERETA</v-btn>
                </v-col>
            </v-row>
        </v-card>
        <v-card elevation="3" style="border-radius: 6px;" class="mt-5 mx-6">
            <v-data-table :headers="headers" :items="Keretas" :search="search">
                <template v-slot:[`item.kelas`]="{item}">
                    <span v-if="item.kelas==1">Ekonomi</span>
                    <span v-if="item.kelas==2">Bisnis</span>
                    <span v-if="item.kelas==3">Eksekutif</span>
                </template>
        ...
        <v-dialog v-model="dialog" width="450">
                    <v-card elevation="3" style="border-radius: 5px;">
                        <v-card-title>
                            <span class="mx-auto my-1">Tambah Kereta</span>
                        </v-card-title>
                        <v-divider></v-divider>
...
<script>
    ...
    export default{
        ...
        methods: {
            ...
            saveData(){
                if(this.formType==0){
                    this.$http.post(this.$api + '/keretas', {
                        name : this.form.name,
                        user_id: 3,
                    ...
                    this.$http.post(this.$api + '/keretas/'+ this.selectedId, {
                        name : this.form.name,
                        from_id: this.form.asal,
                        to_id: this.form.tujuan,
                        kelas: this.form.kelas,
            ...
            deleteBus(id){
                this.$http.delete(this.$api + `/keretas/${id}`)
                .then(response => {
                    this.error_message = '\nBerhasil menambahkan keretas'
                     //this.snackbar = true
                    this.error_message = response.data.message
                    this.reload()
                })
